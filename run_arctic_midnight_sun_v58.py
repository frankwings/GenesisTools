"""v58 — arctic_midnight_sun walkthrough with Held-Karp exact TSP.

Reuses v57 terrain_snake.npz / voxel_grid.npz / walkable.npz and the
Held-Karp path.npz already generated by gen_path_v58.py.
Runs: camera_orient → camera_animate → render → viz figures → GIF/MP4.
"""
import sys
import shutil
...
OUT_DIR = Path("/home/kingy/Projects/Genesis/GenesisTools/results/arctic_midnight_sun_v58")
ASSETS  = Path("/home/kingy/Projects/Genesis/GenesisTools/docs/assets/arctic_midnight_sun_v58")
OUT_DIR.mkdir(parents=True, exist_ok=True)
ASSETS.mkdir(parents=True, exist_ok=True)

# Copy intermediate files from v57 so walkthrough skips steps 1-3
for fname in ("voxel_grid.npz", "walkable.npz"):
    dst = OUT_DIR / fname
    if not dst.exists():
        shutil.copy2(V57 / fname, dst)
        print(f"Copied {fname} from v57")

# terrain_snake.npz + path.npz already present (from gen_path_v58.py)

from genesis_tools.walkthrough_renderer.walkthrough import run

config = {
    "terrain_npz": str(OUT_DIR / "terrain_snake.npz"),
...
print(f"[arctic v58] Walkthrough pipeline → {OUT_DIR}")
result = run(BLEND, config, str(OUT_DIR), render=True)
print("[arctic v58] pipeline done:", result)

# --- Terrain visualizations (figures 0-5) ---
print("[arctic v58] Generating terrain figures …")
...
print(f"[arctic v58] {len(all_frames)} frames → GIF + MP4")
...
gif_plain = ASSETS / "arctic_v58_walkthrough.gif"
create_gif(all_frames, gif_plain, duration=int(1000 / 12))
print(f"[arctic v58] plain GIF → {gif_plain}  ({gif_plain.stat().st_size/1e6:.1f} MB)")

make_combined_gif(
    frames=all_frames,
    path_npz=path_npz,
    terrain_npz=terrain_npz,
    output_gif=ASSETS / "arctic_v58_walkthrough_combined.gif",
...
make_combined_mp4(
    ...
    path_npz=path_npz,
    terrain_npz=terrain_npz,
    output_mp4=ASSETS / "arctic_v58_walkthrough_combined.mp4",
    fps=6, step=1, output_scale=1.0,
)

print("[arctic v58] ALL DONE")
