"""coastal_road — terrain config v1: two-pass TerrainSnake + Held-Karp walkthrough."""
...
OUT_DIR = Path("/home/kingy/Projects/Genesis/GenesisTools/results/coastal_road_terrain_v1")
ASSETS  = Path("/home/kingy/Projects/Genesis/GenesisTools/docs/assets/coastal_road_terrain_v1")
...
if not NPZ.exists():
    print("[coastal_road terrain] Phase 1: fitting terrain snake …")
    subprocess.run([
...
print(f"[coastal_road terrain] {len(all_frames)} frames → GIF + MP4")
...
create_gif(all_frames, ASSETS / "coastal_road_terrain_v1_walkthrough.gif",
           duration=int(1000 / config["fps"]))

make_combined_gif(all_frames, path_npz=OUT_DIR / "path.npz",
                  terrain_npz=NPZ,
                  output_gif=ASSETS / "coastal_road_terrain_v1_walkthrough_combined.gif",
                  fps=config["fps"], step=3, output_scale=0.5)

make_combined_mp4(all_frames, path_npz=OUT_DIR / "path.npz",
                  terrain_npz=NPZ,
                  output_mp4=ASSETS / "coastal_road_terrain_v1_walkthrough_combined.mp4",
                  fps=6, step=1, output_scale=1.0)

print("[coastal_road terrain] ALL DONE")
